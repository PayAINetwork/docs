...
<Note>This page describes x402 v2 over HTTP. For the normative message definitions and newer scheme extensions, see the [upstream v2 specification](https://github.com/x402-foundation/x402/blob/main/specs/x402-specification-v2.md). Protocol support varies by facilitator; check its `/supported` response before choosing a network or scheme.</Note>
...
This reference covers common HTTP integration messages. It is not an exhaustive specification of every transport, payment scheme, or facilitator implementation.
...
| ---------- | ---- | -------- | ----------- |
| `x402Version` | `number` | Required | Set to `2` for this version |
| `error` | `string` | Optional | Explanation for the payment challenge |
| `resource` | `object` | Required | Describes the protected resource |
| `accepts` | `array` | Required | Payment options the server accepts |
| `extensions` | `object` | Optional | Additional protocol features |
...
| `network` | `string` | Required | Blockchain network identifier in CAIP-2 format (e.g., "eip155:84532", "solana:EtWTRABZaYq6iMfeYKouRu166VU2xqa1") |
...
| Field Name | Type | Required | Description |
| ---------- | ---- | -------- | ----------- |
| `x402Version` | `number` | Required | Set to `2` |
| `resource` | `object` | Optional | Resource context echoed by the client |
| `accepted` | `object` | Required | The selected payment option, including `scheme` and `network` |
| `payload` | `object` | Required | Proof or authorization for that scheme |
| `extensions` | `object` | Optional | Extension data returned by the client |

In v2, `scheme` and `network` belong inside `accepted`, not at the top level of the Payment Payload.
...
| `transaction` | `string` | Required | Transaction identifier; a failed or pending result may still include a broadcast transaction |
...
For how listing, refreshing, and the `EXTENSION-RESPONSES` header work on the PayAI facilitator, see <a href="/x402/facilitators/bazaar">Bazaar Discovery</a>.
...
| ---------- | ---- | -------- | ----------- | ------- |
| `limit` | `number` | Optional | Maximum number of results to return (1-1000) | 100 |
  ...
  "items": [
    {
      ...
      "lastUpdated": "2023-12-21T02:17:36.000Z",
      "metadata": {
        "category": "finance",
...
# Fetch the next page
GET /discovery/resources?type=http&limit=10&offset=10
```
...
- **`invalid_exact_svm_payload_transaction_instructions_length`**: The Solana transaction's instruction layout does not meet the selected facilitator's policy. Required compute-budget and transfer instructions, plus supported optional wallet or memo instructions, depend on the SDK and facilitator version (see [§6.2 Exact scheme on Solana](/x402/reference)). Do not assume a universal instruction-count limit.
...
- **Express.js**: [Server middleware guide](/x402/servers/typescript/express)
...
<Note>
Looking for V1 documentation? Visit the [V1 (Legacy) documentation](/v1/x402/reference) section.
</Note>
