...
Use both packages from the [PayAI-owned preview release](https://github.com/PayAINetwork/x402-batch-preview/releases/tag/payai-batch-preview-20260913):
...
Keep the resulting lockfile and verify the release's [SHA256SUMS](https://github.com/PayAINetwork/x402-batch-preview/releases/download/payai-batch-preview-20260913/SHA256SUMS). The tarballs are byte-identical to the original preview, with no package changes. They retain the package names `@x402/core` and `@x402/svm`; they are PayAI preview artifacts, not a stable upstream or npm release. If another dependency pins an older `@x402/core`, use a package-manager override so the preview SVM package resolves the matching core; mixing versions can remove the metered settlement hooks.
