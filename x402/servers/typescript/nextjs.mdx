...
<Tip>
  For production use, add [PayAI API keys](/x402/api-keys) to bypass the free tier settlement limit.
  Install `@payai/facilitator` and set `PAYAI_API_KEY_ID` / `PAYAI_API_KEY_SECRET` env vars.
</Tip>
