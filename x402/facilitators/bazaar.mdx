...
4. **The catalog worker admits the resource.** Before a resource is listed for the first time, the worker checks the URL against the host policy below and probes it once (an unpaid request with your declared method, expecting any HTTP response). Unreachable hosts, `404`/`410` paths, and hosts resolving to private address space are not inserted. Existing entries are refreshed without a probe.
...
Reachability is checked asynchronously by the catalog worker, never on the payment path. Its verdict is visible through [`/discovery/listing-status`](#get-/discovery/listing-status): `resource_unreachable` (DNS failure, connection error, timeout, or a `5xx`) and `resource_not_found` (`404`/`410`).
