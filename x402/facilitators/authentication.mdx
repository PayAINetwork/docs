...
<Note>
  Authentication is required for every `batch-settlement` verification and settlement request, including the first one. Other schemes can use the [free tier](/x402/facilitators/pricing) without an API key. Create a merchant account at [merchant.payai.network](https://merchant.payai.network) to get your API keys.
</Note>
