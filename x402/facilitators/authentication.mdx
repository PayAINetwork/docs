...
<Note>
  Authentication is required for every `batch-settlement` verification and settlement request, including the first one. Ordinary exact payments can use the [free tier](/x402/facilitators/pricing) without an API key. Create a merchant account at [merchant.payai.network](https://merchant.payai.network) to get your API keys. Autonomous agents can also [buy a key over x402 without creating an account](/x402/facilitators/agent-api-keys).
</Note>
...
Send this header on `POST /verify` and `POST /settle`. `GET /supported` is currently readable without a token — it includes batch with `apiKeyRequired: true`; authentication can select a different fee payer for account-specific settlement lanes — so authenticating it is harmless but not required.
...
|----------|--------|------|-------------|
| `/verify` | POST | required for batch; send credentials for authenticated merchant usage | Verify a payment payload against payment requirements |
| `/settle` | POST | required for batch and beyond the ordinary exact free tier | Settle a verified payment on-chain |
...
This is a structural outline, not a valid payment request. Fill both objects using
the complete examples in the [x402 Reference](/x402/reference).
