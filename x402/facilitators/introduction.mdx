...
- Help clients discover merchants via the [Bazaar](/x402/facilitators/bazaar) `/discovery/resources` endpoint
...
For ordinary exact payments, this works on the **free tier** without an API key.
[Solana batch settlement](/x402/servers/batch-settlement) requires a merchant API
key from the first verification or settlement request.
...
To use batch settlement or scale beyond the ordinary exact free tier (up to 1,000 lifetime settlements per receiving wallet, subject to shared-host/IP pools), create a merchant account at [merchant.payai.network](https://merchant.payai.network) for API keys and credits. See [Pricing](/x402/facilitators/pricing) for details.
