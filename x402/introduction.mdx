...
Clients (human users or AI agents) can pay programmatically to access resources
without creating a separate account with every service. Merchant facilitator
credentials are a different layer: PayAI requires them for
[batch settlement](/x402/servers/batch-settlement) and paid facilitator usage.
...
- Payment-based access without a separate customer subscription; applications
  may still require identity or other access controls
