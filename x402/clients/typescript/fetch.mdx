...
This is the `index.ts` the starter generates. It loads your env, wraps `fetch` with x402, calls your endpoint, and logs both the JSON body and the decoded `PAYMENT-RESPONSE` headers.
...
fetchWithPayment(url, {
  ...
  .then(async response => {
    ...
    const paymentResponse = decodeXPaymentResponse(response.headers.get("payment-response")!);
