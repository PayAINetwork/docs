...
This is the `index.ts` the starter generates. It loads your env, attaches the x402 payment interceptor to an Axios instance, calls your endpoint, and logs both the JSON body and the decoded `PAYMENT-RESPONSE` headers.
...
api
  ...
  .then(response => {
    ...
    const paymentResponse = decodeXPaymentResponse(response.headers["payment-response"] as string);
